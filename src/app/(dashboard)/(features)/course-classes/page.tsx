...
export default function CourseClassesPage() {
  ...
  const { data: courseClasses = [], isLoading, error } = useGetCourseClassesQuery(undefined, {
  ...
  // Safe error message extractor without using 'any'
  ...
  const getErrorMessage = (err: unknown, fallback: string): string => {
    if (isRecord(err)) {
      ...
      const directMsg = (err as Record<string, unknown>)['message']
  ...
  const handleModalSubmit = async (data: { name: string; description?: string }) => {
    try {
      ...
      const errorMessage = getErrorMessage(error, 'Failed to save course class. Please try again.')
      toast({
        title: 'Error',
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error, 'Failed to delete course class. Please try again.')
      toast({
        title: 'Error',
        description: errorMessage,
        variant: 'destructive',
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage classes and their descriptions
          </p>
        </div>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} of {courseClasses.length}  classes
        </div>
      </div>
